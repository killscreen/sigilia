...
import net.rowf.sigilia.renderer.shader.program.FlatColorShader;
...
public class EditorActivity extends Activity implements RenderableProvider, RenderableInitializer, OnTouchListener {
	...
	public void onSaveInstanceState(Bundle b) {
		b.putString("EDITING_VERTEXES", dehydrateVertexes());
		b.putString("EDITING_TRIANGLES", dehydrateTriangles());
	...
	public void onRestoreInstanceState(Bundle b) {
		...
		rehydrateVertexes(b.getString("EDITING_VERTEXES"));
		rehydrateVertexes(b.getString("EDITING_TRIANGLES"));
	}
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case 1:
			...
			if (event.getAction() == MotionEvent.ACTION_UP) {
				...
				if (selected != null && (
				    selected.x < -0.5f || selected.x > 0.5f ||
				    selected.y < -0.5f || selected.y > 0.5f)) {
